...
class PerformanceTest(TestCase):
    ...
    @staticmethod
    def print_result_to_console(stats, header) -> None:
        """
        ...
        print("  Top 5 functions with longest execution time:")
        for k, r in stats.nlargest(5, "inlinetime").iterrows():  # noqa: B007
...
class TestDonationBlueprintModel(PerformanceTest):
    def test_blueprint_donation_save_time(self) -> None:
        """
        Test execution times to encrypt and save a donation for varying donation sizes.
        """

        # Load test data donation.
        with Path("../ddm/tests/files_for_tests/test_data_1mb.json").open("rb") as file:
        ...
        for size in [1, 5, 10, 100]:
            ...
            with cProfile.Profile() as profiler:
                ...
                # Get statistics.
                profiler_stats = PerformanceTest.generate_profiler_table(
                    profiler.getstats()
                )
...
class TestDownloadAPIPerformance(PerformanceTest):
    def test_api_download_time(self) -> None:
        ...
        for i in range(1, n_runs + 1):
            ...
            with cProfile.Profile() as profiler:
                ...
                profiler.enable()
                self.client.get(
                    reverse("ddm_apis:project_data", args=[self.project.url_id]),
                    **header,
                )
                profiler.disable()
                ...
                # Print test results to console.
                header = (
                    f"Results for run {i}/{n_runs} with {n_donations} "
                    f"donations à {donation_size}mb"
                )
                PerformanceTest.print_result_to_console(profiler_stats, header)
