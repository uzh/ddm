...
class FileUploaderEditTestCase(TestCase):
    ...
    def test_post_valid_data(self):
        ...
        bp_a_uploader_before = DonationBlueprint.objects.get(
            pk=self.blueprint_a.pk
        ).file_uploader
        bp_b_uploader_before = DonationBlueprint.objects.get(
            pk=self.blueprint_b.pk
        ).file_uploader
        self.client.login(username="owner", password="123")
        response = self.client.post(self.url, valid_data)
        bp_a_uploader_after = DonationBlueprint.objects.get(
            pk=self.blueprint_a.pk
        ).file_uploader
        bp_b_uploader_after = DonationBlueprint.objects.get(
            pk=self.blueprint_b.pk
        ).file_uploader
    ...
    def test_post_invalid_data(self):
        ...
        # Check that the response renders the form with errors
...
class TestAPIs(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.participant_secret = Participant.objects.create(
            project=cls.project_secret, start_time=timezone.now()
        )
