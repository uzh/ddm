...
from ddm.datadonation.models import FileUploader, DonationBlueprint, DonationInstruction
from ddm.participation.models import Participant
from ddm.projects.models import ResearchProfile, DonationProject
from ddm.questionnaire.models import SingleChoiceQuestion, QuestionItem
...
class TestDDMUploaderIntegration(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.briefing_url = reverse(
            'ddm_participation:briefing', args=[project_slug])
        self.dd_url = reverse(
            'ddm_participation:datadonation', args=[project_slug])
        self.quest_url = reverse(
            'ddm_participation:questionnaire', args=[project_slug])
        ...
        try:
            element = WebDriverWait(self.driver, 10).until(
                EC.all_of(
                    ...
                    EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#uapp'), self.blueprint.display_name),
                    EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#uapp'), self.blueprint.description),
                    EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#uapp'), 'custom heading'),
                )
            )
        ...
        try:
            element = WebDriverWait(self.driver, 10).until(
                EC.all_of(
                    ...
                    EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#qapp'), self.sc_question.text),
                    EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#qapp'), self.sc_item.label),
                )
            )
            self.assertIsNotNone(element)
        except Exception as e:
