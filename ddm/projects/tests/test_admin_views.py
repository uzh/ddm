...
class TestAdminViewAuthentication(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.not_permitted_creds = {
            "username": "no_per",
            "password": "123",
            "email": "noperm@liam.com",
        }
        cls.not_permitted_user = User.objects.create_user(**cls.not_permitted_creds)
        cls.not_permitted_profile = ResearchProfile.objects.create(
            user=cls.not_permitted_user
        )

        cls.wo_profile_creds = {
            "username": "no_prof",
            "password": "123",
            "email": "noprof@mail.com",
        }
        ...
        cls.instruction = DonationInstruction.objects.create(
            text="some text", index=1, file_uploader=cls.file_uploader
        )
        ...
        cls.item = QuestionItem.objects.create(
            question=cls.sc_quest, index=1, label="label", value=1
        )

        # Participant
        cls.participant_base = Participant.objects.create(
            project=cls.project_owned, start_time=timezone.now()
        )
    ...
    def get_admin_urls(cls, project_url_id):
        ...
        urls.extend(
            [reverse(view, args=[project_url_id]) for view in project_related_views]
        )
        ...
        urls.extend(
            [
                reverse(view, args=[project_url_id, cls.blueprint.pk])
        ...
        urls.extend(
            [
                reverse(view, args=[project_url_id, cls.file_uploader.pk])
        ...
        urls.extend(
            [
                reverse(
                    view,
                    args=[project_url_id, cls.file_uploader.pk, cls.instruction.pk],
                )
        ...
        for question in questions:
            ...
            urls.append(
                reverse(
                    "ddm_questionnaire:question_filters",
                    args=[project_url_id, question[0], question[1]],
                )
            )
    ...
    def test_user_without_profile_logged_in_creates_profile_and_returns(self):
        for url in self.urls:
            self.assertFalse(
                ResearchProfile.objects.filter(user=self.wo_profile_user).exists()
            )
