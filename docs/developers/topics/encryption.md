...
For some hints on how this should be setup, start with the [Django docs](https://docs.djangoproject.com/en/4.0/topics/security/#ssl-https>).
...
- We then use [PKCS#1 OAEP](https://tools.ietf.org/html/rfc8017) for the wrapping and OAEP padding.

For decryption we need the private key;
- Using the supplied passphrase and salt, we generate a 128bit [HMAC](https://en.wikipedia.org/wiki/HMAC) digest, which we use to populate a pseudo-random number generator.
- This random number generator is used to generate our [RSA](https://en.wikipedia.org/wiki/RSA_(cryptosystem)) key.
- We then use [PKCS#1 OAEP](https://tools.ietf.org/html/rfc8017) for wrapping the OAEP padding.
...
In our case we use [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard>) to encrypt the actual data.
