...
|bp-<X>_time_submitted
|Date and time in ISO 8601 format when the participant has submitted their donation for blueprint X.

|bp-<X>_consent
|Indicates whether the participant has consented to the donation of their data related to blueprint X. `True` = consent, `False` = no consent.

|bp-<X>_extraction_state
|Indicates the status of the participant's donation in relation to blueprint X
("DATA_EXTRACTED" = donation was successful, "NOT_ATTEMPTED" = data donation step reached but donation not attempted,
"FAILED" = donation attempted but donation failed due to an error, "NO_DATA_EXTRACTED" = all data filtered out)

|bp-<X>_n_data_entries
|The number of entries detected in the donated data (depending on the data structure, this value may underestimate how
many entries the donation actually contained).
