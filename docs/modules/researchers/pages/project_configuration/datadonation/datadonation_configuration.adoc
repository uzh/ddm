...
Only necessary, if the Blueprint is associated to an Uploader that expects a ZIP file. This path
...
+
If a regex expression matches more than one file, DDM processes and extracts the data of all matched files consecutively.
...
If you have the extraction of nested files enabled, see the File Uploader settings above for how to define paths to nested files.
====
