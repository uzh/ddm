...
Array join separator:: If an extracted field contains several separate values
...
|conv-2 |Recipe Help |user |How do I make risotto?
|===
...
Nested loop path:: The field that holds the list of sub-items you want to
...
Nested expected fields:: The fields that a sub-item must contain in order
...
Nested expected fields use regex matching:: Select if you use a regex
expression in `Nested expected fields`, the same way
`Expected field regex matching` applies to the top-level `Expected fields`.

Group entries by parent item:: By default, all extracted sub-items (e.g. every
...
Allow item exclusion:: If enabled, participants can remove
