...
DEMO_ZIP_PATH = SCRIPT_DIR / "takeout-demo.zip"
...
docs_module_paths = {
    "ROOT": str(REPO_ROOT / "docs/img") + "/",
    "administrators": str(REPO_ROOT / "docs/administrators/img") + "/",
    "developers": str(REPO_ROOT / "docs/developers/img") + "/",
    "researchers": str(REPO_ROOT / "docs/researchers/img") + "/",
}
...
def get_output_dir(module):
    ...
    target = override.rstrip("/") + "/" if override else docs_module_paths[module]
...
if __name__ == "__main__":
    ...
    Prerequisites:
     ...
     c. A file called 'takeout-demo.zip' must sit next to this script
        (tools/screenshots/) containing a valid data donation for demonstration
        purposes.

    Optional environment variables:
     - DDM_HEADLESS=1        run Firefox headless.
     - DDM_SCREENSHOT_DIR    write all screenshots to this directory instead of
                             docs/<section>/img/ (useful for testing without
                             overwriting the committed screenshots).
    """
    main()
